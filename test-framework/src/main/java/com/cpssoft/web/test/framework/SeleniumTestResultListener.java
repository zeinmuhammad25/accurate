...
public class SeleniumTestResultListener implements IResultListener2 {

	private static final Logger log = Logger.getLogger(SeleniumTestResultListener.class);
	...
	public void onTestFailure(ITestResult result) {
		// AbstractSeleniumTestCase.getWebDriver();
	...
	protected void handleOnError(ITestResult result) {
		...
		try {
			if (AbstractSeleniumTestCase.getWebDriver() != null) {
				// Dump web browser logs
				log.info("Browser Logs");
				log.info("========================");
				LogEntries logEntries = AbstractSeleniumTestCase.getWebDriver().manage().logs().get(LogType.BROWSER);
				...
				log.info("========================");
				logEntries = AbstractSeleniumTestCase.getWebDriver().manage().logs().get(LogType.CLIENT);
				for (LogEntry entry : logEntries) {
					log.info(new Date(entry.getTimestamp()) + " (" + entry.getLevel() + "): " + entry.getMessage());
				}
				...
				log.info("========================");
				logEntries = AbstractSeleniumTestCase.getWebDriver().manage().logs().get(LogType.DRIVER);
				for (LogEntry entry : logEntries) {
					log.info(new Date(entry.getTimestamp()) + " (" + entry.getLevel() + "): " + entry.getMessage());
				}
				...
				try {
					WebDriverTool.takeScreenshot(AbstractSeleniumTestCase.getWebDriver(), result.getTestContext().getOutputDirectory() + "/" + ERROR_SCREENSHOT_FILE);
				} catch (Exception e) {
				...
				try {
					WebDriverTool.dumpPageSource(AbstractSeleniumTestCase.getWebDriver(), result.getTestContext().getOutputDirectory() + "/" + ERROR_PAGE_SOURCE_FILE);
				} catch (Exception e) {
					log.error("Cannot dumping error page source: " + e.getMessage(), e);
				}
			}
		} finally {
			AbstractSeleniumTestCase.getWebDriver().quit();
		}
	}
}
